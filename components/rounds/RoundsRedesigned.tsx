...
export const RoundsRedesigned: React.FC<RoundsRedesignedProps> = ({ patient }) => {
    ...
    const handleScribeToggle = () => {
        if (isRecording) {
            ...
            // In a real app, we would show the badge in the UI, but for simulation we can't easily inject it into the textarea value.
            // Instead, we'll add a visible badge above the form when content is generated.
        } else {
            setIsRecording(true);
        }
    };
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {(soap.subjective || soap.assessment) && (
                                <Badge variant="outline" className="text-[10px] ml-2 border-indigo-200 text-indigo-600 bg-indigo-50">
                                    AI-generated — please verify before signing
                                </Badge>
                            )}
                        </div>
                        <Button
